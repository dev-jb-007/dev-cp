...
void solve(){
	...
	vector<vector<ll>> dp(MAXN+1,vector<ll>(5,-1));
	dp[0][0]=0;
	for(ll i=0;i<MAXN;i++)
	{
		for(ll j=0;j<=4;j++)
		{
			...
			if(j+1<=4)
			{
				if(dev[i+1].first==j+1)
				{
					dp[i+1][j+1]=max(dp[i][j]+dev[i+1].second,dp[i+1][j+1]);
				}
				else{
			...
			if(dev[i+1].first==j)
			{
				dp[i+1][j]=max(dp[i+1][j],dp[i][j]+dev[i+1].second);
			}
			else{
