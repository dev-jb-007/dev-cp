...
void solve(){
	...
	ll n;
	cin>>n;
	map<ll,vector<ll>> vec_x;
	map<ll,vector<ll>> vec_y;
	for(ll i=0;i<n;i++)
	{
		ll a,b;
		cin>>a>>b;
	...
	while(q--)
	{
		...
		if(d=='U')
		{
			ll a=max(1ll,x-len);
			ll b=x;
			ll ct=lower_bound(all(vec_y[y]),a)-vec_y[y].begin();
			if(ct==(vec_y[y].size()))
			{
				x=a;
			}
		...
		else if(d=='D')
		{
			ll a=min(x+len,h);
			ll b=x;
			ll ct=lower_bound(all(vec_y[y]),b)-vec_y[y].begin();
			if(ct==(vec_y[y].size()))
			{
				x=a;
			}
		...
		else if(d=='R')
		{
			ll a=min(y+len,w);
			ll b=y;
			ll ct=lower_bound(all(vec_x[x]),b)-vec_x[x].begin();
			if(ct==(vec_x[x].size()))
			{
				y=a;
			}
			...
			ll a=max(1ll,y-len);
			ll b=y;
			ll ct=lower_bound(all(vec_x[x]),a)-vec_x[x].begin();
			if(ct==(vec_x[x].size()))
			{
				y=a;
			}
