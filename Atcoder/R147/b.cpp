...
void solve(){
	...
	for(int i=n-1;i>=0;i--)
	{
		...
		{
			ll j=i;
			while(j<n-2&&help[j+2]==0)
			{
				ans.PB({'B',j+1});
				j+=2;
				swap(a[j],a[j-2]);
				swap(help[j],help[j-2]);
			}
		}
	}	
	for(int i=0;i<n-1;i++)
	{
		if(help[i]==1&&help[i+1]==1)
		{
			swap(a[i],a[i+1]);
			ans.PB({'A',i+1});
			i++;
	...
	while(1)
	{
		for(int i=0;i<n;i++)
		{
			if(a[i]==i+1)
			{
				...
				ll jumps=(max(a[i]-1,i)-min(a[i]-1,i));
				if(jumps%2==0)
				{
					ll j=min(a[i]-1,i);
					ll m=max(a[i]-1,i);
	...
	for(auto i:ans)
	{
		if(i.first=='A')
		{
			swap(b[i.second-1],b[i.second]);
		}
		else{
			swap(b[i.second-1],b[i.second+1]);
		}
	}
	debug(b);
	for(ll i=1;i<=n;i++)
