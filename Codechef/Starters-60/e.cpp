...
void solve(){
			...
			for(int j=0;j<n;j++)
			{
				if(i!=j)
				{
					if(dev[(a[i]^a[j])]>0)
					{
						...
						for(int k=0;k<n;k++)
						{
							if(k!=i&&k!=j&&k!=(ind[(a[i]^a[j])]-1))
							{
								p=k+1;
								break;
							}
						}
						ans.PB({a[i],a[j],a[p-1]});
						dev[(a[i]^a[j])]++;
						dev[(a[j]^a[p-1])]++;
						dev[(a[p-1]^a[i])]++;
						dev[a[i]]--;
	...
	//main main extra[0] extra[1] extra[2] 
	//0 c c extra[1] extra[2]
	//0 0 d d extra[2]
	// 0 0 0 e e
	debug(main);
	debug(extra);
	ans.PB({main,main,extra[0]});//0
	ans.PB({main^extra[0],main^extra[0],extra[1]});//0
	ans.PB({main^extra[0]^extra[1],main^extra[0]^extra[1],extra[2]});//0
	dev[main]-=2;
	dev[extra[0]]--;
	dev[extra[1]]--;
	dev[extra[2]]--;
	dev[(main^extra[0]^extra[1]^extra[2])]+=2;
	for(auto i:dev)
	{
		if(i.first!=0&&i.second!=0)
		{
			for(ll j=0;j<i.second;j++)
			{
				ans.PB({0,0,i.first});
				ans.PB({0,0,i.first});
				ans.PB({i.first,i.first,i.first});
			}	
		}
	}
	cout<<ans.size()<<endl;
