...
void build(ll m)
{
	for(ll i=m-1;i>=0;i--)
	{
		seg_tree[i]=(calc(seg_tree[2*i])|calc(seg_tree[2*i+1]));
	}
}
ll query(ll l,ll r,ll m)
{
	...
	for(;l<r;l=l/2,r=r/2){
		if(l&1){
			res=(res|(calc(seg_tree[l++])));
		}
		if(r&1)
		{
			res=(res|(calc(seg_tree[--r])));
		}
	}
	return res;
}
void modify(ll pos,ll m,char c)
{
	pos+=m;
	seg_tree[pos]=(1<<(c-'a'));
	for(;pos>1;pos=pos/2)
	{
		seg_tree[pos/2]=(calc(seg_tree[pos])|(calc(seg_tree[pos^1])));
	}
}
void solve(){
	ST s;
