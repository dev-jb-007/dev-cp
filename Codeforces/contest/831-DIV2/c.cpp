...
bool check(ll mid,vl &a)
{
	...
	for(ll i=0;i<n;i++)
	{
		...
		if(i==n-1)
		{
			if(a[n-2]>=temp) return true;
		}
		else{
			if(a[n-1]>=temp) return true;
		}
	}
	debug("3");
	for(ll i=0;i<n;i++)
	{
		...
		if(i==0)
		{
			if(a[1]<=temp) return true;
		}
		else{
			if(a[0]<=temp) return true;
		}
	}
	debug("4");
	return false;
...
void solve(){
    ...
    for(ll i=1;i<n;i++){
        diff=(vec[i]-vec[i-1]);
        ll a=diff+(vec[i]-vec[0]);
        ll b=diff+(vec[n-1]-vec[i-1]);
        ans=max(ans,a);
        ans=max(ans,b);
    }
    cout<<ans<<endl;
