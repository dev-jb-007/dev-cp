...
ll test_case=1;
void permute(ll i,vector<bool> &check,vector<vector<ll>> &perm,ll n,vl a)
{
	if(i==n+1)
	{
		perm.PB(a);
...
void solve(){
	...
	permute(1,check,perm,n,a);
	ll ans=0;
	for(auto i:perm)
	{
		bool dir=true;
		ll curr=0;
		pair<ll,ll> pos={0,1e18};
		//debug(i);
		for(ll j=0;j<(ll)i.size();j++)
		{
			if(vec[i[j]-1][1]<=pos.second)
			{
				if(vec[i[j]-1][0]>=pos.first)
				{
					if(dir)
					{
						curr+=(vec[i[j]-1][2]);
